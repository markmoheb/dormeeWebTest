...
module.exports.acceptRentRequestStatus = (req, res) => {
    Model.getRentRequestById(req.params.id, (err, request) => {
        ...
        let specific = { rentable_id: request.rentable_id, status: 'Pending_Acceptance' };
        let status = { $set: { status: 'Rejected' } };
        ...
        Model.updateRentRequestStatus(query, update, (err) => {
            ...
            Model.rejectOtherRequests(specific, status, (err) => {
...
module.exports.unacceptedStudentRequest = (req, res) => {
    let query = { rentable_id: req.params.id, status: 'Pending_Acceptance' };
    Model.getnAccepted(query, (err, rentRequests) => {
        if (err)
            res.send(err.message);
        else
